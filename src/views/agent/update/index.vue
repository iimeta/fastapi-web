      ...
      <a-card class="general-card" :bordered="false">
        ...
        <div class="wrapper">
          ...
          >
            ...
            <a-step :description="$t('model.agent.subTitle.update.finish')">
              {{ $t('model.agent.title.update.finish') }}
            </a-step>
          </a-steps>
          <keep-alive>
...
<script lang="ts" setup>
  ...
  const submitModelAgent = ref<ModelAgentUpdate>({} as ModelAgentUpdate);

  const submitForm = async () => {
    setLoading(true);
    try {
      await submitModelAgentUpdate(submitModelAgent.value); // The mock api default success
      step.value = 3;
      submitModelAgent.value = {} as ModelAgentUpdate; // init
    } catch (err) {
  ...
  const changeStep = (
    ...
    model: ModelAgentUpdateBaseInfo | ModelAgentUpdate
  ) => {
