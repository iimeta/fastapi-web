  ...
  <div class="chart-card">
    <div class="chart-header">
      <div class="chart-title">{{
        $t('statistics.databoard.agentStatus.title')
      }}</div>
...
<script lang="ts" setup>
  ...
  const d = ref<StatisticsAgentStatusRes>({
    total: 0,
    active: 0,
    disabled: 0,
    auto_disabled: 0,
  ...
  const { chartOption: barOption } = useChartOption((isDark) => ({
    ...
    xAxis: {
      type: 'category',
      data: d.value.by_agent.map((a) => a.name),
      axisLabel: {
        color: isDark ? '#ccc' : '#4E5969',
        fontSize: 11,
        rotate: d.value.by_agent.length > 10 ? 30 : 0,
      },
      axisTick: { show: false },
    ...
    series: [
      {
        type: 'bar',
        barMaxWidth: 14,
        data: d.value.by_agent.map((a) => ({
          value: a.used_quota,
          itemStyle: { color: statusColorMap[a.status] || '#86909C' },
        })),
      },
    ],
  }));
...
<script lang="ts">
  export default { name: 'AgentStatusChart' };
</script>
