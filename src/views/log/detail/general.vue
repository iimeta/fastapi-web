<template>
  <div style="margin: 10px 0 30px 10px">
    ...
    >
      ...
      <a-descriptions-item :label="$t('log.detail.response_data')" :span="2">
        <a-skeleton v-if="loading" :animation="true">
          <a-skeleton-line :rows="1" />
        </a-skeleton>
        <span v-else style="max-height: 220px; display: block; overflow: auto">
...
<script lang="ts" setup>
  ...
  const currentData = ref<GeneralDetail>({} as GeneralDetail);
  ...
  const getGeneralDetail = async (params: DetailParams = { id: props.id }) => {
    setLoading(true);
    try {
      const { data } = await queryGeneralDetail(params);
  ...
  const handleCopyField = async (id: string, field: string) => {
    const { data } = await generalCopyField({ id, field });
    copy(data.value);
  };
