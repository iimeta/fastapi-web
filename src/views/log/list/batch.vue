    ...
    >
      ...
      >
        ...
        <template #width_height="{ record }">
          {{
            record.spend.batch_generation?.pricing?.width
              ? `${record.spend.batch_generation?.pricing.width} × ${record.spend.batch_generation?.pricing.height}`
              : '-'
          }}
        </template>
        <template #seconds="{ record }">
          {{ record.spend.batch_generation?.seconds || '-' }}
        </template>
      ...
      <a-modal
        ...
        :title="$t('log.form.title.batch_del')"
...
<script lang="ts" setup>
  ...
  const delFormData = ref<BatchBatchOperate>({
    status: [1, 2, 3, -1],
  } as BatchBatchOperate);
  ...
  const handleBatch = (params: BatchBatchOperate) => {
    if (ids.value.length === 0 && !params.value) {
      ...
      Modal.warning({
        ...
        onOk: () => {
          ...
          submitBatchBatchOperate(params).then(() => {
            setLoading(false);
