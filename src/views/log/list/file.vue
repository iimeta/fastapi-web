    ...
    >
      ...
      >
        ...
        <template #width_height="{ record }">
          {{
            record.spend.file_generation?.pricing?.width
              ? `${record.spend.file_generation?.pricing.width} × ${record.spend.file_generation?.pricing.height}`
              : '-'
          }}
        </template>
        <template #seconds="{ record }">
          {{ record.spend.file_generation?.seconds || '-' }}
        </template>
      ...
      <a-modal
        ...
        :title="$t('log.form.title.file_del')"
...
<script lang="ts" setup>
  ...
  const delFormData = ref<FileBatchOperate>({
    status: [1, 2, 3, -1],
  } as FileBatchOperate);
  ...
  const handleBatch = (params: FileBatchOperate) => {
    if (ids.value.length === 0 && !params.value) {
      ...
      Modal.warning({
        ...
        onOk: () => {
          ...
          submitFileBatchOperate(params).then(() => {
            setLoading(false);
...
<style scoped lang="less">
  ...
  @import '../style/log-list-copy-shared.less';
