  ...
  <div class="container">
    ...
    >
      ...
      <a-modal
        ...
        :title="$t('log.form.title.audio_del')"
...
<script lang="ts" setup>
  ...
  const delFormData = ref<AudioBatchOperate>({
    status: [1, 2, 3, -1],
  } as AudioBatchOperate);
  ...
  const handleBatch = (params: AudioBatchOperate) => {
    if (ids.value.length === 0 && !params.value) {
      ...
      Modal.warning({
        ...
        onOk: () => {
          ...
          submitAudioBatchOperate(params).then(() => {
            setLoading(false);
