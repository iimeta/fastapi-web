<template>
  <div class="container">
    <a-breadcrumb class="container-breadcrumb">
      ...
      <a-breadcrumb-item>{{ $t('log.menu.video') }}</a-breadcrumb-item>
    </a-breadcrumb>
    <a-card
      class="general-card"
      :bordered="false"
    ...
    >
      ...
      <a-modal
        ...
        :title="$t('log.form.title.video_del')"
...
<script lang="ts" setup>
  ...
  const columns = computed<TableColumnData[]>(() => [
    ...
    {
      title: t('log.columns.width_height'),
  ...
  const delFormData = ref<VideoBatchOperate>({
    status: [1, 2, 3, -1],
  } as VideoBatchOperate);
    ...
    if (ids.value.length === 0 && !params.value) {
      ...
      Modal.warning({
        ...
        onOk: () => {
          ...
          submitVideoBatchOperate(params).then(() => {
            setLoading(false);
