export interface ProviderSvgLogo {
  kind: 'svg';
  viewBox: string;
  path?: string;
  color?: string;
  markup?: string;
}

export type ProviderLogo = ProviderSvgLogo;

interface ProviderAsset {
  aliases: string[];
  color: string;
  logo?: ProviderLogo;
}

function makeSvgLogo(svg: string): ProviderSvgLogo {
  const viewBoxMatch = svg.match(/viewBox="([^"]+)"/);
  const markupMatch = svg.match(/<svg[^>]*>([\s\S]*)<\/svg>/);

  return {
    kind: 'svg',
    viewBox: viewBoxMatch?.[1] || '0 0 64 64',
    markup: markupMatch?.[1]?.trim() || '',
  };
}

function makeWordmarkLogo(
  text: string,
  background: string,
  foreground = '#ffffff',
  size = 22
) {
  return makeSvgLogo(`
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
      <rect width="64" height="64" rx="16" fill="${background}" />
      <text
        x="32"
        y="36"
        text-anchor="middle"
        dominant-baseline="middle"
        font-family="Arial, Helvetica, sans-serif"
        font-size="${size}"
        font-weight="700"
        fill="${foreground}"
      >${text}</text>
    </svg>
  `);
}

const googleLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <path d="M54.7 32.6c0-1.8-.2-3.5-.5-5.2H32v10h12.7c-.6 3.2-2.4 5.9-5.1 7.7v6.4h8.2c4.8-4.4 7.5-11 7.5-18.9z" fill="#4285F4"/>
    <path d="M32 55.5c6.5 0 11.9-2.2 15.8-5.9l-8.2-6.4c-2.3 1.5-5.1 2.4-7.6 2.4-5.9 0-10.9-4-12.7-9.4h-8.5v6.6c3.9 7.7 11.8 12.7 21.2 12.7z" fill="#34A853"/>
    <path d="M19.3 36.2c-.5-1.5-.8-3-.8-4.7s.3-3.2.8-4.7v-6.6h-8.5A23.5 23.5 0 0 0 8.4 31.5c0 3.8.9 7.4 2.4 10.7l8.5-6z" fill="#FBBC04"/>
    <path d="M32 18.5c3.5 0 6.7 1.2 9.2 3.5l6.9-6.9C43.9 11.2 38.5 9 32 9c-9.4 0-17.3 5-21.2 12.7l8.5 6.6c1.8-5.4 6.8-9.8 12.7-9.8z" fill="#EA4335"/>
  </svg>
`);

const geminiLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <defs>
      <linearGradient id="g" x1="12%" y1="10%" x2="88%" y2="90%">
        <stop offset="0%" stop-color="#4285F4"/>
        <stop offset="32%" stop-color="#7B61FF"/>
        <stop offset="68%" stop-color="#A24DFF"/>
        <stop offset="100%" stop-color="#00C7B1"/>
      </linearGradient>
    </defs>
    <path d="M32 7c3.7 14.2 10.8 21.3 25 25-14.2 3.7-21.3 10.8-25 25-3.7-14.2-10.8-21.3-25-25 14.2-3.7 21.3-10.8 25-25z" fill="url(#g)"/>
    <path d="M32 15.5c2.4 9.4 7.1 14.1 16.5 16.5-9.4 2.4-14.1 7.1-16.5 16.5-2.4-9.4-7.1-14.1-16.5-16.5 9.4-2.4 14.1-7.1 16.5-16.5z" fill="#fff" opacity=".18"/>
  </svg>
`);

const claudeLogo = makeSvgLogo(`
  <svg height="1em" style="flex:none;line-height:1" viewBox="0 0 24 24" width="1em" xmlns="http://www.w3.org/2000/svg"><title>Claude</title><path d="M4.709 15.955l4.72-2.647.08-.23-.08-.128H9.2l-.79-.048-2.698-.073-2.339-.097-2.266-.122-.571-.121L0 11.784l.055-.352.48-.321.686.06 1.52.103 2.278.158 1.652.097 2.449.255h.389l.055-.157-.134-.098-.103-.097-2.358-1.596-2.552-1.688-1.336-.972-.724-.491-.364-.462-.158-1.008.656-.722.881.06.225.061.893.686 1.908 1.476 2.491 1.833.365.304.145-.103.019-.073-.164-.274-1.355-2.446-1.446-2.49-.644-1.032-.17-.619a2.97 2.97 0 01-.104-.729L6.283.134 6.696 0l.996.134.42.364.62 1.414 1.002 2.229 1.555 3.03.456.898.243.832.091.255h.158V9.01l.128-1.706.237-2.095.23-2.695.08-.76.376-.91.747-.492.584.28.48.685-.067.444-.286 1.851-.559 2.903-.364 1.942h.212l.243-.242.985-1.306 1.652-2.064.73-.82.85-.904.547-.431h1.033l.76 1.129-.34 1.166-1.064 1.347-.881 1.142-1.264 1.7-.79 1.36.073.11.188-.02 2.856-.606 1.543-.28 1.841-.315.833.388.091.395-.328.807-1.969.486-2.309.462-3.439.813-.042.03.049.061 1.549.146.662.036h1.622l3.02.225.79.522.474.638-.079.485-1.215.62-1.64-.389-3.829-.91-1.312-.329h-.182v.11l1.093 1.068 2.006 1.81 2.509 2.33.127.578-.322.455-.34-.049-2.205-1.657-.851-.747-1.926-1.62h-.128v.17l.444.649 2.345 3.521.122 1.08-.17.353-.608.213-.668-.122-1.374-1.925-1.415-2.167-1.143-1.943-.14.08-.674 7.254-.316.37-.729.28-.607-.461-.322-.747.322-1.476.389-1.924.315-1.53.286-1.9.17-.632-.012-.042-.14.018-1.434 1.967-2.18 2.945-1.726 1.845-.414.164-.717-.37.067-.662.401-.589 2.388-3.036 1.44-1.882.93-1.086-.006-.158h-.055L4.132 18.56l-1.13.146-.487-.456.061-.746.231-.243 1.908-1.312-.006.006z" fill="#D97757" fill-rule="nonzero"></path></svg>
`);
const anthropicLogo = makeSvgLogo(`
  <svg role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <title>Anthropic</title>
    <path d="M17.3041 3.541h-3.6718l6.696 16.918H24Zm-10.6082 0L0 20.459h3.7442l1.3693-3.5527h7.0052l1.3693 3.5528h3.7442L10.5363 3.5409Zm-.3712 10.2232 2.2914-5.9456 2.2914 5.9456Z" fill="#D97757"/>
  </svg>
`);

const deepSeekLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 377.1 277.86">
    <path fill="#4d6bfe" d="M373.15,23.32c-4-1.95-5.72,1.77-8.06,3.66-.79.62-1.47,1.43-2.14,2.14-5.85,6.26-12.67,10.36-21.57,9.86-13.04-.71-24.16,3.38-33.99,13.37-2.09-12.31-9.04-19.66-19.6-24.38-5.54-2.45-11.13-4.9-14.99-10.23-2.71-3.78-3.44-8-4.81-12.16-.85-2.51-1.72-5.09-4.6-5.52-3.13-.5-4.36,2.14-5.58,4.34-4.93,8.99-6.82,18.92-6.65,28.97.43,22.58,9.97,40.56,28.89,53.37,2.16,1.46,2.71,2.95,2.03,5.09-1.29,4.4-2.82,8.68-4.19,13.09-.85,2.82-2.14,3.44-5.15,2.2-10.39-4.34-19.37-10.76-27.29-18.55-13.46-13.02-25.63-27.41-40.81-38.67-3.57-2.64-7.12-5.09-10.81-7.41-15.49-15.07,2.03-27.45,6.08-28.9,4.25-1.52,1.47-6.79-12.23-6.73-13.69.06-26.24,4.65-42.21,10.76-2.34.93-4.79,1.61-7.32,2.14-14.5-2.73-29.55-3.35-45.29-1.58-29.62,3.32-53.28,17.34-70.68,41.28C1.29,88.2-3.63,120.88,2.39,155c6.33,35.91,24.64,65.68,52.8,88.94,29.18,24.1,62.8,35.91,101.15,33.65,23.29-1.33,49.23-4.46,78.48-29.24,7.38,3.66,15.12,5.12,27.97,6.23,9.89.93,19.41-.5,26.79-2.02,11.55-2.45,10.75-13.15,6.58-15.13-33.87-15.78-26.44-9.36-33.2-14.54,17.21-20.41,43.15-41.59,53.3-110.19.79-5.46.11-8.87,0-13.3-.06-2.67.54-3.72,3.61-4.03,8.48-.96,16.72-3.29,24.28-7.47,21.94-12,30.78-31.69,32.87-55.33.31-3.6-.06-7.35-3.86-9.24ZM181.96,235.97c-32.83-25.83-48.74-34.33-55.31-33.96-6.14.34-5.04,7.38-3.69,11.97,1.41,4.53,3.26,7.66,5.85,11.63,1.78,2.64,3.01,6.57-1.78,9.49-10.57,6.58-28.95-2.2-29.82-2.64-21.38-12.59-39.26-29.24-51.87-52.01-12.16-21.92-19.23-45.43-20.39-70.52-.31-6.08,1.47-8.22,7.49-9.3,7.92-1.46,16.11-1.77,24.03-.62,33.49,4.9,62.01,19.91,85.9,43.63,13.65,13.55,23.97,29.71,34.61,45.49,11.3,16.78,23.48,32.75,38.97,45.84,5.46,4.59,9.83,8.09,14,10.67-12.59,1.4-33.62,1.71-47.99-9.68ZM197.69,134.65c0-2.7,2.15-4.84,4.87-4.84.6,0,1.16.12,1.66.31.67.25,1.29.62,1.77,1.18.87.84,1.36,2.08,1.36,3.35,0,2.7-2.15,4.84-4.85,4.84s-4.81-2.14-4.81-4.84ZM246.55,159.77c-3.13,1.27-6.26,2.39-9.27,2.51-4.67.22-9.77-1.68-12.55-4-4.3-3.6-7.36-5.61-8.67-11.94-.54-2.7-.23-6.85.25-9.24,1.12-5.15-.12-8.44-3.74-11.44-2.96-2.45-6.7-3.1-10.82-3.1-1.54,0-2.95-.68-4-1.24-1.72-.87-3.13-3.01-1.78-5.64.43-.84,2.53-2.92,3.02-3.29,5.58-3.19,12.03-2.14,18,.25,5.54,2.26,9.71,6.42,15.72,12.28,6.16,7.1,7.26,9.09,10.76,14.39,2.76,4.19,5.29,8.47,7.01,13.37,1.04,3.04-.31,5.55-3.94,7.1Z"/>
  </svg>
`);

const baiduLogo = makeSvgLogo(`
  <svg version="1.2" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1386 1514">
    <path d="m449.1 1079.4h92.8v209.5h-84.8c0 0-61-2.7-79.6-71.6-10.6-29.1 0-66.3 5.4-79.6 5.3-15.8 23.8-47.7 66.2-58.3z" fill="#2830e1"/>
    <path fill="#2830e1" fill-rule="evenodd" d="m767.3 246.6c10.5-87.5 119.3-225.4 204.1-206.9 84.8 18.6 164.4 132.6 148.5 230.8-16 98.1-92.8 228-214.7 212.2-122.1-16-148.5-127.4-137.9-236.1zm-254.5 220.1c-90.9 0-164.4-104.3-164.4-233.3 0-129.1 73.5-233.4 164.4-233.4 90.9 0 164.4 104.3 164.4 233.4 0 129-73.5 233.3-164.4 233.3zm-509.1 87.6c0 0 18.6-193.7 151.2-204.2 106-10.6 182.9 106 190.9 172.3 5.3 42.4 26.4 238.8-137.9 273.2-161.8 34.5-222.8-153.9-204.2-241.3zm1222.2 344.8c-180.2 2.6-188.2-122.1-188.2-212.2 0-95.5 18.5-228.1 164.3-228.1 143.2 0 183 140.5 183 188.3 0 45.1 18.5 246.7-159.1 252zm29.2 485.3c-90.2 209.5-418.9 100.7-418.9 100.7 0 0-121.9-39.7-262.4-7.9-140.6 29.2-262.6 18.6-262.6 18.6 0 0-164.3 2.6-212-204.3-45.1-209.5 164.3-323.5 180.2-342.1 16-21.2 127.3-95.5 196.3-212.1 68.8-119.4 281-212.2 429.5 18.5 108.6 156.5 296.8 302.3 296.8 302.3 0 0 140.5 108.8 53.1 326.3zm-612.5-10.6v-517.2l-95.4-2.7v140.6h-114c0 0-114 7.9-153.8 137.9-15.9 87.5 10.7 137.9 16 148.5 2.6 10.6 39.7 74.3 132.5 92.9zm392.4-2.7v-363.3h-106v273.1h-100.8c0 0-31.8-5.3-37.1-31.8v-244l-100.7 2.7v270.5c0 0 5.3 68.9 100.7 92.8z"/>
  </svg>
`);

const wenxinLogo = makeSvgLogo(`
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="wenxin-gradient" x1="9.155%" x2="90.531%" y1="75.177%" y2="25.028%">
        <stop offset="0%" stop-color="#0A51C3"></stop>
        <stop offset="100%" stop-color="#23A4FB"></stop>
      </linearGradient>
    </defs>
    <path d="M11.32 1.176a1.4 1.4 0 011.36 0l8.64 4.843c.421.234.68.67.68 1.141v9.68c0 .472-.259.908-.68 1.143l-8.64 4.84a1.4 1.4 0 01-1.36 0l-8.64-4.84A1.31 1.31 0 012 16.84V7.159c0-.471.259-.907.68-1.142l8.64-4.84zm7.42 13.839V8.227L12.002 12 12 19.551l6.059-3.394a1.31 1.31 0 00.68-1.142zM12.68 4.833a1.393 1.393 0 00-1.36 0L5.944 7.846c-.421.235-.68.67-.68 1.142v6.027c0 .47.259.905.68 1.142l2.795 1.566V11.09a1.546 1.546 0 00.221.79 1.527 1.527 0 01-.216-.834l.004-.094.02-.15.018-.084.017-.062.039-.117.062-.142.035-.065.081-.13.094-.122.084-.091.08-.075.125-.1.071-.048.134-.076 5.87-3.29-2.796-1.566z" fill="url(#wenxin-gradient)"></path>
    <path d="M12 11.088c0-.875-.73-1.584-1.631-1.584a1.66 1.66 0 00-.855.237c-.027.016-.055.033-.08.05a2.361 2.361 0 00-.123.093c-.022.02-.045.038-.066.059l-.048.045-.063.067c-.014.016-.028.031-.04.048a2.303 2.303 0 00-.094.125l-.042.069a1.7 1.7 0 00-.07.13l-.036.081a.764.764 0 00-.022.06c-.01.03-.02.058-.028.087l-.017.062a.883.883 0 00-.03.16c-.002.025-.007.05-.008.074a1.527 1.527 0 00.213.929c.302.508.85.792 1.414.792.277 0 .558-.068.814-.212l.815-.457v-.914L12 11.088z" fill="#012F8D"></path>
  </svg>
`);

const metaLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <path d="M10 41c0-12.4 5.9-20.7 13.8-20.7 6.9 0 10.1 6.9 14.3 13.8 2.1 3.3 4.4 6.7 7.2 6.7 3 0 5.3-3.2 8.5-10.8 0 10.7-4.7 17.9-11.3 17.9-6.3 0-9.6-6.3-13-12.5-2.6-4.8-4.8-8.4-7.1-8.4-3.1 0-5.4 5-5.4 14 0 4-7 4-7 0zm44-.1c0-12.4-5.9-20.6-13.8-20.6-6.9 0-10.1 6.9-14.3 13.8 1 1.8 2.1 3.5 3.1 5.2 3.4-5.7 5.8-12.1 10.9-12.1 4.6 0 8.1 5.8 8.1 13.7 0 4 6 4 6 0z" fill="#0866FF"/>
  </svg>
`);

const mistralLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <rect width="64" height="64" rx="16" fill="#FFF2E8"/>
    <path d="M14 48V16h8v12h6V16h8v12h6V16h8v32h-8V36h-6v12h-8V36h-6v12h-8z" fill="#FF7000"/>
  </svg>
`);

const qwenLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="27.55 17.52 147.28 145.51">
    <defs>
      <radialGradient
        id="qwen-gradient-primary"
        cx="0"
        cy="0"
        r="1"
        gradientUnits="userSpaceOnUse"
        gradientTransform="translate(100 100) rotate(90) scale(100)"
      >
        <stop stop-color="#665CEE"/>
        <stop offset="1" stop-color="#332E91"/>
      </radialGradient>
      <radialGradient
        id="qwen-gradient-secondary"
        cx="0"
        cy="0"
        r="1"
        gradientUnits="userSpaceOnUse"
        gradientTransform="translate(100 100) rotate(90) scale(100)"
      >
        <stop stop-color="#665CEE"/>
        <stop offset="1" stop-color="#332E91"/>
      </radialGradient>
    </defs>
    <path
      d="M174.82 108.75L155.38 75L165.64 57.75C166.46 56.31 166.46 54.53 165.64 53.09L155.38 35.84C154.86 34.91 153.87 34.33 152.78 34.33H114.88L106.14 19.03C105.62 18.1 104.63 17.52 103.54 17.52H83.3C82.21 17.52 81.22 18.1 80.7 19.03L61.26 52.77H41.02C39.93 52.77 38.94 53.35 38.42 54.28L28.16 71.53C27.34 72.97 27.34 74.75 28.16 76.19L45.52 107.5L36.78 122.8C35.96 124.24 35.96 126.02 36.78 127.46L47.04 144.71C47.56 145.64 48.55 146.22 49.64 146.22H87.54L96.28 161.52C96.8 162.45 97.79 163.03 98.88 163.03H119.12C120.21 163.03 121.2 162.45 121.72 161.52L141.16 127.78H158.52C159.61 127.78 160.6 127.2 161.12 126.27L171.38 109.02C172.2 107.58 172.2 105.8 171.38 104.36L174.82 108.75Z"
      fill="url(#qwen-gradient-primary)"
    />
    <path
      d="M119.12 163.03H98.88L87.54 144.71H49.64L61.26 126.39H80.7L38.42 55.29H61.26L83.3 19.03L93.56 37.35L83.3 55.29H161.58L151.32 72.54L170.76 106.28H151.32L141.16 88.34L101.18 163.03H119.12Z"
      fill="#FFFFFF"
    />
    <path
      d="M127.86 79.83H76.14L101.18 122.11L127.86 79.83Z"
      fill="url(#qwen-gradient-secondary)"
    />
  </svg>
`);

const tencentLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <rect width="64" height="64" rx="16" fill="#F2F8FF"/>
    <path d="M14 31c2-8.9 9.8-15 18.9-15 6.2 0 12 2.9 15.8 7.8l-6.8 1.3A13.4 13.4 0 0 0 32.3 21c-5.9 0-11 3.8-12.8 9.5L14 31z" fill="#006EFF"/>
    <path d="M19.5 33.5 34 22.5 22.3 47.8c-6 0-10.3-4.2-10.3-9.8 0-1.8.5-3.3 1.4-4.5l6.1 0z" fill="#00C8DC"/>
    <path d="M22.8 47.8 40.5 24.2c1.8-2.3 4.2-3.4 6.9-3.4 5.1 0 9.6 4.2 9.6 9.7 0 1.8-.5 3.3-1.4 4.7L46.7 47.8H22.8z" fill="#00A3FF"/>
  </svg>
`);

const minimaxLogo = makeWordmarkLogo('MM', '#FF4D4F', '#FFFFFF', 22);
const zhipuLogo = makeSvgLogo(`
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path d="M11.991 23.503a.24.24 0 00-.244.248.24.24 0 00.244.249.24.24 0 00.245-.249.24.24 0 00-.22-.247l-.025-.001zM9.671 5.365a1.697 1.697 0 011.099 2.132l-.071.172-.016.04-.018.054c-.07.16-.104.32-.104.498-.035.71.47 1.279 1.186 1.314h.366c1.309.053 2.338 1.173 2.286 2.523-.052 1.332-1.152 2.38-2.478 2.327h-.174c-.715.018-1.274.64-1.239 1.368 0 .124.018.23.053.337.209.373.54.658.96.8.75.23 1.517-.125 1.9-.782l.018-.035c.402-.64 1.17-.96 1.92-.711.854.284 1.378 1.226 1.099 2.167a1.661 1.661 0 01-2.077 1.102 1.711 1.711 0 01-.907-.711l-.017-.035c-.2-.323-.463-.58-.851-.711l-.056-.018a1.646 1.646 0 00-1.954.746 1.66 1.66 0 01-1.065.764 1.677 1.677 0 01-1.989-1.279c-.209-.906.332-1.83 1.257-2.043a1.51 1.51 0 01.296-.035h.018c.68-.071 1.151-.622 1.116-1.333a1.307 1.307 0 00-.227-.693 2.515 2.515 0 01-.366-1.403 2.39 2.39 0 01.366-1.208c.14-.195.21-.444.227-.693.018-.71-.506-1.261-1.186-1.332l-.07-.018a1.43 1.43 0 01-.299-.07l-.05-.019a1.7 1.7 0 01-1.047-2.114 1.68 1.68 0 012.094-1.101zm-5.575 10.11c.26-.264.639-.367.994-.27.355.096.633.379.728.74.095.362-.007.748-.267 1.013-.402.41-1.053.41-1.455 0a1.062 1.062 0 010-1.482zm14.845-.294c.359-.09.738.024.992.297.254.274.344.665.237 1.025-.107.36-.396.634-.756.718-.551.128-1.1-.22-1.23-.781a1.05 1.05 0 01.757-1.26zm-.064-4.39c.314.32.49.753.49 1.206 0 .452-.176.886-.49 1.206-.315.32-.74.5-1.185.5-.444 0-.87-.18-1.184-.5a1.727 1.727 0 010-2.412 1.654 1.654 0 012.369 0zm-11.243.163c.364.484.447 1.128.218 1.691a1.665 1.665 0 01-2.188.923c-.855-.36-1.26-1.358-.907-2.228a1.68 1.68 0 011.33-1.038c.593-.08 1.183.169 1.547.652zm11.545-4.221c.368 0 .708.2.892.524.184.324.184.724 0 1.048a1.026 1.026 0 01-.892.524c-.568 0-1.03-.47-1.03-1.048 0-.579.462-1.048 1.03-1.048zm-14.358 0c.368 0 .707.2.891.524.184.324.184.724 0 1.048a1.026 1.026 0 01-.891.524c-.569 0-1.03-.47-1.03-1.048 0-.579.461-1.048 1.03-1.048zm10.031-1.475c.925 0 1.675.764 1.675 1.706s-.75 1.705-1.675 1.705-1.674-.763-1.674-1.705c0-.942.75-1.706 1.674-1.706zm-2.626-.684c.362-.082.653-.356.761-.718a1.062 1.062 0 00-.238-1.028 1.017 1.017 0 00-.996-.294c-.547.14-.881.7-.752 1.257.13.558.675.907 1.225.783zm0 16.876c.359-.087.644-.36.75-.72a1.062 1.062 0 00-.237-1.019 1.018 1.018 0 00-.985-.301 1.037 1.037 0 00-.762.717c-.108.361-.017.754.239 1.028.245.263.606.377.953.305l.043-.01zM17.19 3.5a.631.631 0 00.628-.64c0-.355-.279-.64-.628-.64a.631.631 0 00-.628.64c0 .355.28.64.628.64zm-10.38 0a.631.631 0 00.628-.64c0-.355-.28-.64-.628-.64a.631.631 0 00-.628.64c0 .355.279.64.628.64zm-5.182 7.852a.631.631 0 00-.628.64c0 .354.28.639.628.639a.63.63 0 00.627-.606l.001-.034a.62.62 0 00-.628-.64zm5.182 9.13a.631.631 0 00-.628.64c0 .355.279.64.628.64a.631.631 0 00.628-.64c0-.355-.28-.64-.628-.64zm10.38.018a.631.631 0 00-.628.64c0 .355.28.64.628.64a.631.631 0 00.628-.64c0-.355-.279-.64-.628-.64zm5.182-9.148a.631.631 0 00-.628.64c0 .354.279.639.628.639a.631.631 0 00.628-.64c0-.355-.28-.64-.628-.64zm-.384-4.992a.24.24 0 00.244-.249.24.24 0 00-.244-.249.24.24 0 00-.244.249c0 .142.122.249.244.249zM11.991.497a.24.24 0 00.245-.248A.24.24 0 0011.99 0a.24.24 0 00-.244.249c0 .133.108.236.223.247l.021.001zM2.011 6.36a.24.24 0 00.245-.249.24.24 0 00-.244-.249.24.24 0 00-.244.249.24.24 0 00.244.249zm0 11.263a.24.24 0 00-.243.248.24.24 0 00.244.249.24.24 0 00.244-.249.252.252 0 00-.244-.248zm19.995-.018a.24.24 0 00-.245.248.24.24 0 00.245.25.24.24 0 00.244-.25.252.252 0 00-.244-.248z" fill="#3859FF" fill-rule="nonzero"></path>
  </svg>
`);
const chatglmLogo = makeSvgLogo(`
  <svg height="1em" style="flex:none;line-height:1" viewBox="0 0 24 24" width="1em" xmlns="http://www.w3.org/2000/svg"><title>ChatGLM</title><defs><linearGradient id="lobe-icons-chat-glm-_R_0_" x1="-18.756%" x2="70.894%" y1="49.371%" y2="90.944%"><stop offset="0%" stop-color="#504AF4"></stop><stop offset="100%" stop-color="#3485FF"></stop></linearGradient></defs><path d="M9.917 2c4.906 0 10.178 3.947 8.93 10.58-.014.07-.037.14-.057.21l-.003-.277c-.083-3-1.534-8.934-8.87-8.934-3.393 0-8.137 3.054-7.93 8.158-.04 4.778 3.555 8.4 7.95 8.332l.073-.001c1.2-.033 2.763-.429 3.1-1.657.063-.031.26.534.268.598.048.256.112.369.192.34.981-.348 2.286-1.222 1.952-2.38-.176-.61-1.775-.147-1.921-.347.418-.979 2.234-.926 3.153-.716.443.102.657.38 1.012.442.29.052.981-.2.96.242-1.5 3.042-4.893 5.41-8.808 5.41C3.654 22 0 16.574 0 11.737 0 5.947 4.959 2 9.917 2zM9.9 5.3c.484 0 1.125.225 1.38.585 3.669.145 4.313 2.686 4.694 5.444.255 1.838.315 2.3.182 1.387l.083.59c.068.448.554.737.982.516.144-.075.254-.231.328-.47a.2.2 0 01.258-.13l.625.22a.2.2 0 01.124.238 2.172 2.172 0 01-.51.92c-.878.917-2.757.664-3.08-.62-.14-.554-.055-.626-.345-1.242-.292-.621-1.238-.709-1.69-.295-.345.315-.407.805-.406 1.282L12.6 15.9a.9.9 0 01-.9.9h-1.4a.9.9 0 01-.9-.9v-.65a1.15 1.15 0 10-2.3 0v.65a.9.9 0 01-.9.9H4.8a.9.9 0 01-.9-.9l.035-3.239c.012-1.884.356-3.658 2.47-4.134.2-.045.252.13.29.342.025.154.043.252.053.294.701 3.058 1.75 4.299 3.144 3.722l.66-.331.254-.13c.158-.082.25-.131.276-.15.012-.01-.165-.206-.407-.464l-1.012-1.067a8.925 8.925 0 01-.199-.216c-.047-.034-.116.068-.208.306-.074.157-.251.252-.272.326-.013.058.108.298.362.72.164.288.22.508-.31.343-1.04-.8-1.518-2.273-1.684-3.725-.004-.035-.162-1.913-.162-1.913a1.2 1.2 0 011.113-1.281L9.9 5.3zm12.994 8.68c.037.697-.403.704-1.213.591l-1.783-.276c-.265-.053-.385-.099-.313-.147.47-.315 3.268-.93 3.31-.168zm-.915-.083l-.926.042c-.85.077-1.452.24.338.336l.103.003c.815.012 1.264-.359.485-.381zm1.667-3.601h.01c.79.398.067 1.03-.65 1.393-.14.07-.491.176-1.052.315-.241.04-.457.092-.333.16l.01.005c1.952.958-3.123 1.534-2.495 1.285l.38-.148c.68-.266 1.614-.682 1.666-1.337.038-.48 1.253-.442 1.493-.968.048-.106 0-.236-.144-.389-.05-.047-.094-.094-.107-.148-.073-.305.7-.431 1.222-.168zm-2.568-.474c-.135 1.198-2.479 4.192-1.949 2.863l.017-.042c.298-.717.376-2.221 1.337-3.221.25-.26.636.035.595.4zm-7.976-.253c.02-.694 1.002-.968 1.346-.347.01-1.274-1.941-.768-1.346.347z" fill="url(#lobe-icons-chat-glm-_R_0_)" fill-rule="evenodd"></path></svg>
`);

const xfyunLogo = makeSvgLogo(`
  <svg height="1em" style="flex:none;line-height:1" viewBox="0 0 24 24" width="1em" xmlns="http://www.w3.org/2000/svg">
    <title>Spark</title>
    <path d="M2 13.08C2 9.182 4.772 6.367 9.32 2.122c-.65 7.883 6.41 8.272 5.023 12.214-.99 2.815-4.244 1.949-4.59 1.342 0 0 1.212.347 1.385-.866.174-1.213-2.252-1.862-3.81-4.937-2.6 2.988-.954 9.008 4.2 9.008 4.764 0 6.583-4.937 4.894-8.099 0 0 4.071.693 4.418 3.811.346 3.119-3.638 8.533-9.095 8.403C6.288 22.868 2 18.84 2 13.08z" fill="#3DC8F9"></path>
    <path d="M17.852 6.107L11.615 0c-.52 5.933.866 8.374 4.894 9.485 2.729.753 3.307 1.04 4.504 2.772-.338-2.407-.78-3.812-3.161-6.15z" fill="#EA0100"></path>
    <path clip-rule="evenodd" d="M9.033 18.323c.709.354 1.542.56 2.495.56 4.764 0 6.583-4.937 4.894-8.099 0 0 4.071.693 4.418 3.811.156 1.403-.565 3.27-1.902 4.89-3.458 1.57-7.29.84-9.905-1.162z" fill="#1652D8" fill-rule="evenodd"></path>
  </svg>
`);
const openRouterLogo = makeSvgLogo(`
  <svg viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
    <path d="M3 248.945C18 248.945 76 236 106 219C136 202 136 202 198 158C276.497 102.293 332 120.945 423 120.945" stroke="#111111" stroke-width="90" fill="none"/>
    <path d="M511 121.5L357.25 210.268L357.25 32.7324L511 121.5Z" fill="#111111"/>
    <path d="M0 249C15 249 73 261.945 103 278.945C133 295.945 133 295.945 195 339.945C273.497 395.652 329 377 420 377" stroke="#111111" stroke-width="90" fill="none"/>
    <path d="M508 376.445L354.25 287.678L354.25 465.213L508 376.445Z" fill="#111111"/>
  </svg>
`);
const cohereLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 118 20">
    <g clip-path="url(#cohere-logo-clip)">
      <path fill="#355146" d="M6.81969 11.9081C7.34472 11.9081 8.3891 11.8785 9.8327 11.2692C11.5149 10.559 14.8619 9.2699 17.2762 7.94576C18.9648 7.01963 19.7049 5.79474 19.7049 4.14523C19.705 1.85589 17.8949 0 15.6621 0H6.30704C3.09989 0 0.5 2.66571 0.5 5.95406C0.5 9.2424 2.93427 11.9081 6.81969 11.9081Z" clip-rule="evenodd" fill-rule="evenodd"/>
      <path fill="#D18EE2" d="M8.40234 16.0108C8.40234 14.3989 9.34877 12.9456 10.8008 12.3277L13.7469 11.0741C16.7269 9.80599 20.0069 12.0514 20.0069 15.3595C20.0069 17.9226 17.9801 20.0001 15.4803 19.9994L12.2906 19.9985C10.143 19.998 8.40234 18.2128 8.40234 16.0108Z" clip-rule="evenodd" fill-rule="evenodd"/>
      <path fill="#FF7759" d="M3.8476 12.6914H3.84753C1.99874 12.6914 0.5 14.2281 0.5 16.1237V16.5683C0.5 18.4639 1.99874 20.0005 3.84753 20.0005H3.8476C5.69639 20.0005 7.19513 18.4639 7.19513 16.5683V16.1237C7.19513 14.2281 5.69639 12.6914 3.8476 12.6914Z"/>
    </g>
    <defs>
      <clipPath id="cohere-logo-clip">
        <rect width="19.5062" height="20" fill="white" transform="translate(0.5)"/>
      </clipPath>
    </defs>
  </svg>
`);
const togetherLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 484.98 452.5">
    <path d="M468.72,60.67C435.25,2.69,361.11-17.18,303.12,16.3c-37.3,21.54-58.82,59.62-60.52,99.69l121.15.16v10.39h-121.15c.78,18.94,6.02,37.81,16.15,55.36,33.48,57.98,107.62,77.85,165.6,44.37,57.98-33.48,77.85-107.62,44.37-165.6Z" fill="#ef2cc1"/>
    <path d="M16.26,60.63C-17.21,118.61,2.65,192.76,60.63,226.23c37.3,21.54,81.04,21.13,116.59,2.57l-60.44-105,9-5.18,60.57,104.91c16.01-10.14,29.74-24.12,39.87-41.67,33.48-57.98,13.61-132.12-44.37-165.6C123.88-17.21,49.74,2.65,16.26,60.63Z" fill="#caaef5"/>
    <path d="M242.46,452.5c66.95,0,121.23-54.27,121.23-121.23,0-43.07-22.22-80.75-56.07-102.25l-60.71,104.84-8.99-5.21,60.57-104.91c-16.79-8.79-35.75-13.7-56.02-13.7-66.95,0-121.23,54.27-121.23,121.23,0,66.95,54.27,121.23,121.23,121.23Z" fill="#fc4c02"/>
  </svg>
`);
const xLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
    <rect width="64" height="64" rx="16" fill="#111111"/>
    <path d="M19 16h8.9l9.2 12.2L47 16h5L39.3 30.6 53 48H44l-9.8-13.1L22.5 48h-5l14.1-15.9z" fill="#fff"/>
  </svg>
`);
const azureLogo = makeSvgLogo(`
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="azure-gradient-left" x1="8.247" x2="1.002" y1="1.626" y2="23.03" gradientUnits="userSpaceOnUse">
        <stop stop-color="#114A8B"></stop>
        <stop offset="1" stop-color="#0669BC"></stop>
      </linearGradient>
      <linearGradient id="azure-gradient-shadow" x1="14.042" x2="12.324" y1="15.302" y2="15.888" gradientUnits="userSpaceOnUse">
        <stop stop-opacity=".3"></stop>
        <stop offset=".071" stop-opacity=".2"></stop>
        <stop offset=".321" stop-opacity=".1"></stop>
        <stop offset=".623" stop-opacity=".05"></stop>
        <stop offset="1" stop-opacity="0"></stop>
      </linearGradient>
      <linearGradient id="azure-gradient-right" x1="12.841" x2="20.793" y1="1.626" y2="22.814" gradientUnits="userSpaceOnUse">
        <stop stop-color="#3CCBF4"></stop>
        <stop offset="1" stop-color="#2892DF"></stop>
      </linearGradient>
    </defs>
    <path d="M7.242 1.613A1.11 1.11 0 018.295.857h6.977L8.03 22.316a1.11 1.11 0 01-1.052.755h-5.43a1.11 1.11 0 01-1.053-1.466L7.242 1.613z" fill="url(#azure-gradient-left)"></path>
    <path d="M18.397 15.296H7.4a.51.51 0 00-.347.882l7.066 6.595c.206.192.477.298.758.298h6.226l-2.706-7.775z" fill="#0078D4"></path>
    <path d="M15.272.857H7.497L0 23.071h7.775l1.596-4.73 5.068 4.73h6.665l-2.707-7.775h-7.998L15.272.857z" fill="url(#azure-gradient-shadow)"></path>
    <path d="M17.193 1.613a1.11 1.11 0 00-1.052-.756h-7.81.035c.477 0 .9.304 1.052.756l6.748 19.992a1.11 1.11 0 01-1.052 1.466h-.12 7.895a1.11 1.11 0 001.052-1.466L17.193 1.613z" fill="url(#azure-gradient-right)"></path>
  </svg>
`);
const moonshotLogo = makeWordmarkLogo('K', '#4F46E5', '#FFFFFF', 28);
const doubaoLogo = makeWordmarkLogo('DB', '#3370FF', '#FFFFFF', 20);
const volcengineLogo = makeSvgLogo(`
  <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    <path d="M19.44 10.153l-2.936 11.586a.215.215 0 00.214.261h5.87a.215.215 0 00.214-.261l-2.95-11.586a.214.214 0 00-.412 0zM3.28 12.778l-2.275 8.96A.214.214 0 001.22 22h4.532a.212.212 0 00.214-.165.214.214 0 000-.097l-2.276-8.96a.214.214 0 00-.41 0z" fill="#00E5E5"></path>
    <path d="M7.29 5.359L3.148 21.738a.215.215 0 00.203.261h8.29a.214.214 0 00.215-.261L7.7 5.358a.214.214 0 00-.41 0z" fill="#006EFF"></path>
    <path d="M14.44.15a.214.214 0 00-.41 0L8.366 21.739a.214.214 0 00.214.261H19.9a.216.216 0 00.171-.078.214.214 0 00.044-.183L14.439.15z" fill="#006EFF"></path>
    <path d="M10.278 7.741L6.685 21.736a.214.214 0 00.214.264h7.17a.215.215 0 00.214-.264L10.688 7.741a.214.214 0 00-.41 0z" fill="#00E5E5"></path>
  </svg>
`);

const fastapiLogo = makeSvgLogo(`
  <svg xmlns="http://www.w3.org/2000/svg" width="512" height="514" viewBox="0 0 512 514" role="img">
    <image href="/logo.png" width="512" height="514" preserveAspectRatio="xMidYMid meet" />
  </svg>
`);

const providerAssets: ProviderAsset[] = [
  {
    aliases: ['openai', 'chatgpt', 'gpt', 'codex'],
    color: '#111111',
    logo: {
      kind: 'svg',
      viewBox: '0 0 24 24',
      path: 'M22.282 9.821a5.985 5.985 0 0 0-.516-4.91 6.046 6.046 0 0 0-6.51-2.9A6.065 6.065 0 0 0 4.981 4.18a5.985 5.985 0 0 0-3.998 2.9 6.046 6.046 0 0 0 .743 7.097 5.98 5.98 0 0 0 .51 4.911 6.051 6.051 0 0 0 6.515 2.9A5.985 5.985 0 0 0 13.26 24a6.056 6.056 0 0 0 5.772-4.206 5.99 5.99 0 0 0 3.997-2.9 6.056 6.056 0 0 0-.747-7.073zM13.26 22.43a4.476 4.476 0 0 1-2.876-1.04l.141-.081 4.779-2.758a.795.795 0 0 0 .392-.681v-6.737l2.02 1.168a.071.071 0 0 1 .038.052v5.583a4.504 4.504 0 0 1-4.494 4.494zM3.6 18.304a4.47 4.47 0 0 1-.535-3.014l.142.085 4.783 2.759a.771.771 0 0 0 .78 0l5.843-3.369v2.332a.08.08 0 0 1-.033.062L9.74 19.95a4.5 4.5 0 0 1-6.14-1.646zM2.34 7.896a4.485 4.485 0 0 1 2.366-1.973V11.6a.766.766 0 0 0 .388.676l5.815 3.355-2.02 1.168a.076.076 0 0 1-.071 0l-4.83-2.786A4.504 4.504 0 0 1 2.34 7.872zm16.597 3.855l-5.833-3.387L15.119 7.2a.076.076 0 0 1 .071 0l4.83 2.791a4.494 4.494 0 0 1-.676 8.105v-5.678a.79.79 0 0 0-.407-.667zm2.01-3.023l-.141-.085-4.774-2.782a.776.776 0 0 0-.785 0L9.409 9.23V6.897a.066.066 0 0 1 .028-.061l4.83-2.787a4.5 4.5 0 0 1 6.68 4.66zm-12.64 4.135l-2.02-1.164a.08.08 0 0 1-.038-.057V6.075a4.5 4.5 0 0 1 7.375-3.453l-.142.08L8.704 5.46a.795.795 0 0 0-.393.681zm1.097-2.365l2.602-1.5 2.607 1.5v2.999l-2.597 1.5-2.607-1.5z',
      color: '#111111',
    },
  },
  { aliases: ['anthropic'], color: '#d97757', logo: claudeLogo },
  { aliases: ['claude', 'cc'], color: '#d97757', logo: claudeLogo },
  { aliases: ['gemini', 'gcpgemini'], color: '#4285f4', logo: geminiLogo },
  { aliases: ['google'], color: '#4285f4', logo: geminiLogo },
  {
    aliases: [
      'deepseek',
      'deep-seek',
      'deepseek-ai',
      'deepseek baidu',
      'deepseek-baidu',
    ],
    color: '#4d6bfe',
    logo: deepSeekLogo,
  },
  {
    aliases: ['wenxin', '文心', 'ernie', 'erniebot', 'ernie bot'],
    color: '#2d41ff',
    logo: wenxinLogo,
  },
  {
    aliases: ['baidu', 'qianfan', 'wenxinyiyan', 'wenxin yiyan', '文心一言'],
    color: '#2d41ff',
    logo: wenxinLogo,
  },
  {
    aliases: ['azure', 'azure openai', 'azureopenai', 'az'],
    color: '#0078d4',
    logo: azureLogo,
  },
  { aliases: ['meta', 'llama'], color: '#0082fb', logo: metaLogo },
  { aliases: ['mistral'], color: '#ff7000', logo: mistralLogo },
  {
    aliases: [
      'aliyun',
      'alibaba',
      'alibabacloud',
      'dashscope',
      'tongyi',
      'qwen',
      '阿里云',
      '通义',
    ],
    color: '#ff6a00',
    logo: qwenLogo,
  },
  {
    aliases: ['tencent', 'hunyuan', '腾讯', '混元'],
    color: '#0052d9',
    logo: tencentLogo,
  },
  { aliases: ['minimax'], color: '#ff4d4f', logo: minimaxLogo },
  {
    aliases: [
      'zhipu',
      'zhipuai',
      'zhipu ai',
      'bigmodel',
      'z.ai',
      '智谱',
      '智谱AI',
    ],
    color: '#3478f6',
    logo: zhipuLogo,
  },
  {
    aliases: ['glm', 'glm-4', 'chatglm', '智谱清言'],
    color: '#3485ff',
    logo: chatglmLogo,
  },
  {
    aliases: ['xunfei', 'xfyun', 'spark', '讯飞', '星火'],
    color: '#00a9ff',
    logo: xfyunLogo,
  },
  {
    aliases: ['openrouter'],
    color: '#6f4cff',
    logo: openRouterLogo,
  },
  {
    aliases: ['cohere', 'command', 'command-r', 'command-r-plus'],
    color: '#39594d',
    logo: cohereLogo,
  },
  {
    aliases: ['together', 'togetherai', 'together ai'],
    color: '#7c3aed',
    logo: togetherLogo,
  },
  {
    aliases: ['groq'],
    color: '#f55036',
  },
  {
    aliases: ['perplexity', 'pplx'],
    color: '#20b8a6',
  },
  {
    aliases: ['replicate'],
    color: '#111111',
  },
  {
    aliases: ['sambanova', 'samba nova', 'sambanova ai'],
    color: '#e11d48',
  },
  {
    aliases: ['01ai', '01.ai', 'yi', 'yi-large', 'yi-lightning', '零一万物'],
    color: '#0f766e',
  },
  {
    aliases: ['stepfun', 'step', 'step-1', 'step-2', '阶跃', '阶跃星辰'],
    color: '#2563eb',
  },
  {
    aliases: [
      'baichuan',
      'baichuan-ai',
      'baichuan2',
      'baichuan4',
      '百川',
      '百川智能',
    ],
    color: '#2563eb',
  },
  {
    aliases: ['sensetime', 'sensenova', 'sensechat', '日日新', '商汤'],
    color: '#ef4444',
  },
  { aliases: ['x', 'xai', 'grok'], color: '#111111', logo: xLogo },
  {
    aliases: ['moonshot', 'kimi', '月之暗面'],
    color: '#4f46e5',
    logo: moonshotLogo,
  },
  { aliases: ['doubao', '豆包'], color: '#3370ff', logo: doubaoLogo },
  {
    aliases: ['volcengine', '火山引擎'],
    color: '#ff6b3d',
    logo: volcengineLogo,
  },
  {
    aliases: ['智元', 'FastAPI'],
    color: '#993D92',
    logo: fastapiLogo,
  },
];

const fallbackPalette = [
  '#6366f1',
  '#8b5cf6',
  '#ec4899',
  '#14b8a6',
  '#f59e0b',
  '#06b6d4',
];

function normalizeProviderKey(providerCode?: string, providerName?: string) {
  return `${providerCode || ''} ${providerName || ''}`.trim().toLowerCase();
}

function findAsset(
  assets: ProviderAsset[],
  providerCode?: string,
  providerName?: string
) {
  const key = normalizeProviderKey(providerCode, providerName);
  return assets.find((item) =>
    item.aliases.some((alias) => key.includes(alias.toLowerCase()))
  );
}

const providerCompanyAssets: ProviderAsset[] = [
  {
    aliases: ['openai', 'chatgpt'],
    color: '#10a37f',
    logo: providerAssets[0].logo,
  },
  { aliases: ['anthropic'], color: '#d97757', logo: anthropicLogo },
  { aliases: ['claude'], color: '#d97757', logo: claudeLogo },
  {
    aliases: ['google', 'gemini', 'gcpgemini'],
    color: '#4285f4',
    logo: googleLogo,
  },
  {
    aliases: [
      'deepseek',
      'deep-seek',
      'deepseek-ai',
      'deepseek baidu',
      'deepseek-baidu',
    ],
    color: '#4d6bfe',
    logo: deepSeekLogo,
  },
  {
    aliases: [
      'baidu',
      'qianfan',
      'wenxin',
      'wenxinyiyan',
      'wenxin yiyan',
      'ernie',
      'erniebot',
      'ernie bot',
      '文心',
      '文心一言',
    ],
    color: '#2d41ff',
    logo: baiduLogo,
  },
  {
    aliases: ['azure', 'azure openai', 'azureopenai'],
    color: '#0078d4',
    logo: azureLogo,
  },
  { aliases: ['meta', 'llama'], color: '#0082fb', logo: metaLogo },
  { aliases: ['mistral'], color: '#ff7000', logo: mistralLogo },
  {
    aliases: ['tencent', 'hunyuan', '腾讯', '混元'],
    color: '#0052d9',
    logo: tencentLogo,
  },
  { aliases: ['minimax'], color: '#ff4d4f', logo: minimaxLogo },
  {
    aliases: [
      'zhipu',
      'zhipuai',
      'zhipu ai',
      'bigmodel',
      'z.ai',
      '智谱',
      '智谱AI',
    ],
    color: '#3478f6',
    logo: zhipuLogo,
  },
  {
    aliases: ['xunfei', 'xfyun', '讯飞', '科大讯飞'],
    color: '#00a9ff',
    logo: xfyunLogo,
  },
  {
    aliases: ['openrouter'],
    color: '#6f4cff',
    logo: openRouterLogo,
  },
  {
    aliases: ['cohere'],
    color: '#39594d',
    logo: cohereLogo,
  },
  {
    aliases: ['together', 'togetherai', 'together ai'],
    color: '#7c3aed',
    logo: togetherLogo,
  },
  {
    aliases: ['groq'],
    color: '#f55036',
  },
  {
    aliases: ['perplexity', 'pplx'],
    color: '#20b8a6',
  },
  {
    aliases: ['replicate'],
    color: '#111111',
  },
  {
    aliases: ['sambanova', 'samba nova', 'sambanova ai'],
    color: '#e11d48',
  },
  {
    aliases: ['01ai', '01.ai', '零一万物'],
    color: '#0f766e',
  },
  {
    aliases: ['stepfun', '阶跃', '阶跃星辰'],
    color: '#2563eb',
  },
  {
    aliases: ['baichuan', 'baichuan-ai', '百川', '百川智能'],
    color: '#2563eb',
  },
  {
    aliases: ['sensetime', 'sensenova', '商汤'],
    color: '#ef4444',
  },
  { aliases: ['x', 'xai', 'grok'], color: '#111111', logo: xLogo },
  {
    aliases: ['moonshot', 'kimi', '月之暗面'],
    color: '#4f46e5',
    logo: moonshotLogo,
  },
  {
    aliases: ['volcengine', '火山引擎'],
    color: '#ff6b3d',
    logo: volcengineLogo,
  },
  {
    aliases: [
      'aliyun',
      'alibaba',
      'alibabacloud',
      'dashscope',
      'tongyi',
      'qwen',
      '阿里云',
      '通义',
    ],
    color: '#ff6a00',
    logo: qwenLogo,
  },
  {
    aliases: ['智元', 'FastAPI'],
    color: '#993D92',
    logo: fastapiLogo,
  },
];

function findProviderAsset(providerCode?: string, providerName?: string) {
  return findAsset(providerAssets, providerCode, providerName);
}

function findProviderCompanyAsset(
  providerCode?: string,
  providerName?: string
) {
  return findAsset(providerCompanyAssets, providerCode, providerName);
}

export function getProviderLogo(providerCode?: string, providerName?: string) {
  return findProviderAsset(providerCode, providerName)?.logo || null;
}

export function getProviderBrandColor(
  providerCode?: string,
  providerName?: string
) {
  const asset = findProviderAsset(providerCode, providerName);
  if (asset) return asset.color;

  const key = normalizeProviderKey(providerCode, providerName);
  let hash = 0;
  for (let i = 0; i < key.length; i += 1) {
    hash = key.charCodeAt(i) + (hash * 31 - hash);
  }
  return fallbackPalette[Math.abs(hash) % fallbackPalette.length];
}

export function getProviderInitial(providerName?: string) {
  return providerName ? providerName.charAt(0).toUpperCase() : '?';
}

export function getProviderCompanyLogo(
  providerCode?: string,
  providerName?: string
) {
  return findProviderCompanyAsset(providerCode, providerName)?.logo || null;
}

export function getProviderCompanyBrandColor(
  providerCode?: string,
  providerName?: string
) {
  const asset = findProviderCompanyAsset(providerCode, providerName);
  if (asset) return asset.color;

  return getProviderBrandColor(providerCode, providerName);
}
